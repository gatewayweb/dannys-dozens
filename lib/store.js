...
const getOrderTotal = (cart) => {
  ...
  if (cart && cart.length) {
    cart.forEach((cartItem) => {
      ...
      if (size === 'Dozen') {
        pricePerItem = parseFloat(item.pricing.pricePerDozen);
      } else {
        pricePerItem = parseFloat(item.pricing.pricePerHalfDozen);
      }
        ...
        if (state.cart && state.cart.length) {
          state.cart.forEach((cartItem) => {
            ...
            if (size === 'Dozen') {
              ...
              pricePerItem = parseFloat(item.pricing.pricePerSingle);
            }
